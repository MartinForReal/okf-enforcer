...
- **Live check on save / open**, **Scan vault on startup**, **Fix format issues on save**, **Auto-generate index.md**, **Auto-migrate to latest OKF on fix** — automation toggles. Every folder gets an `index.md` — a new or empty one included, generated as soon as the folder appears, and left empty while there is nothing to list — and an existing one is kept current as notes are added, renamed, and deleted, along with every listing above it, since a parent describes its subdirectories by what they hold.
- **Rebuild existing index.md** — off by default, so generating over an existing index adds what's missing, fixes links that point at the wrong path, and removes entries for notes that are gone, changing nothing else. Turn it on to rewrite the listing from the folder's contents instead, which also refreshes every description, re-sorts the entries, and re-groups them under their current `type`.
