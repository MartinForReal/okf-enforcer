...
- **`index.md` generation.** Index files are optional — §8 says one *may* appear in a directory and §11 forbids failing a bundle for a missing one — so a folder with something to list gets an index generated rather than flagged, and one that exists is validated against §8. Generating over an existing index is **additive**: the entries it doesn't already list are appended, a link that points at the wrong path is corrected, and your prose, ordering, titles, and edited descriptions stay as written. Listings follow §8 (with `okf_version` in the root index): concept entries carry their frontmatter `description` — as §8 recommends; subdirectory entries always link to that folder's own `index.md`, never a bare `folder/` path that Obsidian would turn into a new note, and can pull their description from a named section (e.g. `# Purpose`) of that index. A folder with nothing to enumerate gets no index at all, and no entry in its parent's listing either, so no entry points at an index that will never exist.
...
- **Default actor for `generated.by`** — the actor recorded when auto-fix adds a `generated` block (e.g. `okf-enforcer/0.4` or `human:<id>`).
- **Live check on save / open**, **Scan vault on startup**, **Fix format issues on save**, **Auto-generate index.md**, **Auto-migrate to latest OKF on fix** — automation toggles. A folder with something to list gets an `index.md`; an existing one has its missing entries appended and its wrong links corrected.
- **Rebuild existing index.md** — off by default, so generating over an existing index adds what's missing and fixes links that point at the wrong path, and changes nothing else. Turn it on to rewrite the listing from the folder's contents instead, which also drops entries for notes that no longer exist.
- **Subdirectory description section** — heading in a subfolder's `index.md` whose first paragraph becomes that folder's description in the parent listing (e.g. `Purpose`). Blank by default, which leaves subdirectory entries undescribed. Additive generation never touches the section, and a rebuild carries it over, so the description you write survives a refresh either way.
- **Batch size** — files processed per async chunk (lower = smoother UI on very large vaults).
- **Warn on missing recommended fields / tags**, **Validate trust & lifecycle fields**, **Validate Attested Computation concepts** — which optional checks to surface. `index.md` / `log.md` structure is always checked: §11 makes it one of the three requirements for a conformant bundle, so it isn't a toggle.
