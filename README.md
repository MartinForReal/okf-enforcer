...
- **Live check on save / open**, **Scan vault on startup**, **Fix format issues on save**, **Auto-generate index.md**, **Auto-migrate to latest OKF on fix** — automation toggles. A folder with something to list gets an `index.md`; an existing one is kept current as notes are added, renamed, and deleted, and so is every listing above it, since a parent describes its subdirectories by what they hold. Generating a folder also writes the indexes of the subfolders it links at, however deep, so a listing never points at a file that isn't there.
