...
- **Live check on save / open**, **Scan vault on startup**, **Fix format issues on save**, **Auto-generate index.md**, **Auto-migrate to latest OKF on fix** — automation toggles. A folder with something to list gets an `index.md`; an existing one has its missing entries appended.
- **Rebuild existing index.md** — off by default, so generating over an existing index only adds what's missing. Turn it on to rewrite the listing from the folder's contents instead, which also drops entries for notes that no longer exist.
